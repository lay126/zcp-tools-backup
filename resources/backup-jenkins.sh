...
time kubectl exec -it $POD -- tar -zcf /appdata/$BACKUP_NAME.tgz /var/jenkins_home || echo 'tar done' > /appdata/$BACKUP_NAME.log
echo -e '\n\n+ [backup] end Compress...'

while [ ! -f $POD:/appdata/$BACKUP_NAME.log ]; do sleep 2s; done
